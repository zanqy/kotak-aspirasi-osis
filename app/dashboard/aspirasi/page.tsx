  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        else { router.push("/dashboard/login"); return; }
      } catch {}
  ...
  return (
    <div className="min-h-screen" style={{ background: 'var(--paper)' }}>
      <div className="max-w-[900px] mx-auto pb-20 px-4 pt-6">
        ...
        {/* Folder tabs - kategori sebagai tab map arsip */}
        <motion.div
        ...
        {/* Folder body - list aspirasi */}
        <div className="bg-card border-[1.5px] border-line rounded-r-md rounded-b-md shadow-paper relative" style={{ animation: 'unfold 0.7s cubic-bezier(0.22,1,0.36,1) forwards' }}>
          <div className="relative z-[1] py-1">
            {loading ? (
              ...
              <div className="text-center py-16 text-ink-faint">
                ...
                <p className="text-[12.5px]">Tidak ada aspirasi{filterStatus ? ` dengan status ${filterStatus}` : ""}</p>
              </div>
            ) : (
              filtered.map((item, i) => (
                <motion.div
